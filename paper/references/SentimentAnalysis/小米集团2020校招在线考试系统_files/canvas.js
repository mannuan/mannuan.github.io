    ...
    (function () {
        ...
        */
        ...
        $("#container").mousedown(function (e) {
            ...
            switch (command) {
                ...
                case 5: {
                    ...
                    var myrgba = "";
                    if (rectalpha == 100) {
                        myrgba = "rgba(" + backgroundColor.colorRgb() + ",1)";
                    } else if (rectalpha == 0) {
                        myrgba = "rgba(" + backgroundColor.colorRgb() + ",1)";
                    } else {
                        myrgba = "rgba(" + backgroundColor.colorRgb() + "," + parseFloat('0.' + rectalpha) + ")";
                    }

                    rectTip.css({
                        "border": sr,
                        "background-color": myrgba
                    });
                    rectTip.show();
                    break;
                }
        ...
        function drawRectangle() {
            ...
            ctx.fillStyle = myrgba;

            ctx.fillRect(x + parseInt(borderWidth), y + parseInt(borderWidth), endX - x - parseInt(borderWidth), endY - y - parseInt(borderWidth));

            ctx.strokeRect(x + parseInt(borderWidth)/2, y + parseInt(borderWidth)/2, endX - x - parseInt(borderWidth), endY - y - parseInt(borderWidth));
            $("#myCanvas").focus();
            rectTip.hide();
        }
        ...
        function initUI() {
            ...
            function fillColorEventListener(e) {
                ...
                rectTip.css({ "background-color": myrgba });
        ...
        $(".savepainterimg").click(saveItAsImage);
        ...
        function saveItAsImage() {
            ...
            //alert("图片上传成功！");
            var img = '<img src="{0}" border="0" alter="jPainter" />'.Format(image);
            var iframe;
            try{
                iframe = $("iframe").contents().find("body.cke_editable");
            }
            catch (e){
                iframe = $($(".cke_reset iframe")[0].contentDocument.body);
            }
            var imgOld = $(iframe).find('img[src^="data:"]');
            var oldImg = null;
            ...
            if (oldImg)
                ...
                $(iframe).append(img);
            //$('#txtAnswer').html(html.join(''));
            //如果是已经加载了编辑器
            /*if ($(".fulltexteditor").hasClass("sel")) {
                var html = editor.getData();
                $("#txtAnswer").html(html);
                $('#txtAnswer img').remove();
                var html1 = [$('#txtAnswer').html()];
                html1.push(img);
                //$('body.cke_editable').html(html);
                //alert(html1);
                editor.setData(html1);
                //CKEDITOR.instances.setData(html1);
            } else {
                $('#txtAnswer img').remove();
                var html = [$('#txtAnswer').html()];
                html.push(img);
                $('#txtAnswer').html(html);
            }                      */
